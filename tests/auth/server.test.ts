...
describe('OAuth Callback Server', () => {
  test('starts on random port and receives auth code', async () => {
    const { result, port } = await startOAuthCallbackServer(0)
    expect(port).toBeGreaterThan(0)

    const response = await fetch(`http://127.0.0.1:${port}/callback?code=test-auth-code&state=test-state`)
  ...
  test('handles code without state', async () => {
    const { result, port } = await startOAuthCallbackServer(0)
    await fetch(`http://127.0.0.1:${port}/callback?code=no-state-code`)
    const callbackResult = await result
